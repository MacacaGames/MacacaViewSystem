/// Credit lordofduct
/// Sourced from - https://github.com/lordofduct/spacepuppy-unity-framework/blob/master/SpacepuppyBase/Tween/Easing.cs
...
namespace com.spacepuppy.Tween
{
    ...
    public static class EaseMethods
    {
        ...
        // public static float EaseFloat(Ease ease, float start, float end, float t, float dur)
        // {
        ...
        public static IEnumerator EaseVector3(Vector3 start, Vector3 end, float dur, Ease ease, Action<Vector3> OnUpdate, Action OnComplete = null)
        {
            ...
            {
                float sc = ease(t, 0f, 1f, dur);
                OnUpdate?.Invoke(Vector3.LerpUnclamped(start, end, sc));
                yield return null;
                t += Time.deltaTime;
            }
            OnUpdate?.Invoke(end);
